...
const navItems = [
  { id: 'dialpad', label: 'شماره‌گیر', icon: (_active: boolean) => (
  ...
  { id: 'contacts', label: 'مخاطبین', icon: (_active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
      <circle cx="12" cy="7" r="4"/>
    </svg>
  )},
  { id: 'history', label: 'تماس‌ها', icon: (_active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
    </svg>
  )},
  { id: 'settings', label: 'تنظیمات', icon: (_active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  { id: 'autofill', label: 'فرم', icon: (_active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
export function Sidebar({ activePage, onNavigate }: SidebarProps) {
  ...
  return (
    <aside className="w-16 flex flex-col items-center py-3 bg-bg-surface border-e border-border gap-1">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all duration-200 ${
              isActive
                ? 'bg-accent/10 text-accent shadow-[0_0_12px_var(--c-accent-glow)]'
                : 'hover-overlay text-text-secondary'
            }`}
            title={item.label}
          >
            {item.icon(isActive)}
      ...
      <div className="flex flex-col items-center gap-2 pb-2">
        ...
        <span className="text-[10px] text-text-muted">
          {sipStatus === 'registered' ? 'آنلاین' : sipStatus === 'connecting' ? '...' : 'آفلاین'}
        </span>
      </div>
    </aside>
  )
